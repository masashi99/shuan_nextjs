...
// 授業を登録できるコマのIDのみを抽出
export const CLASS_PERIOD_IDS = PERIODS.filter(
	(p) => !p.isBreak && !p.isMemo,
).map((p) => p.id);
...
export function PlannerProvider({ children }: { children: ReactNode }) {
	...
	const deleteSubject = (id: string) => {
		...
		setClassTemplates(
			classTemplates.filter((template) => template.subjectId !== id),
		);
	};
	...
	const addUnit = (
		...
		setUnits([
			...units,
			{ id: Date.now().toString(), subjectId, name, requiredPeriods },
		]);
	};
	...
	const updateUnit = (id: string, name: string, requiredPeriods: number) => {
		...
		setUnits(
			units.map((unit) =>
				unit.id === id ? { ...unit, name, requiredPeriods } : unit,
			),
		);
	};
	...
	const deleteUnit = (id: string) => {
		...
		setClassTemplates(
			classTemplates.map((template) =>
				template.unitId === id ? { ...template, unitId: null } : template,
			),
		);
	};

	// 授業テンプレートの設定（追加または更新）
	const setClassTemplate = (
		day: string,
		periodId: number,
		subjectId: string,
		unitId: string | null,
	) => {
		const existingIndex = classTemplates.findIndex(
