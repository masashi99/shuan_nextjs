...
import { THEME_COLORS, usePlanner } from "@/contexts/planner-context";
...
export function UnitSettings() {
	...
	const saveEditingUnit = () => {
		if (editingUnit?.name?.trim() && editingUnit.requiredPeriods > 0) {
	...
	const getColorClass = (colorName: string) => {
		return (
			THEME_COLORS.find((color) => color.name === colorName)?.value ||
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
					{subjects.map((subject) => {
						const subjectUnits = units.filter(
							(unit) => unit.subjectId === subject.id,
						);
										...
										{!editingSubject && (
											<div className="flex space-x-2">
												<Button
													...
													size="icon"
													onClick={() =>
														startEditingSubject(subject.id, subject.name)
													}
												>
