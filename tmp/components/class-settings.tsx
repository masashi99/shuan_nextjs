...
export function ClassSettings() {
  ...
  const [selectedCell, setSelectedCell] = useState<{ day: string; periodId: number } | null>(null)
  ...
  const saveClassTemplate = () => {
    if (selectedCell && selectedSubjectId) {
      setClassTemplate(selectedCell.day, selectedCell.periodId, selectedSubjectId, selectedUnitId)
  ...
  const displayPeriods = PERIODS.filter((period) => !period.isBreak && !period.isMemo)
